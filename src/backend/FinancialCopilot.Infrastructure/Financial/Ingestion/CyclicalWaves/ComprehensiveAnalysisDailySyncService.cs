...
internal sealed class ComprehensiveAnalysisDailySyncService(
    ...
    ILogger<ComprehensiveAnalysisDailySyncService> logger)
    : IComprehensiveAnalysisDailySyncService
{
    public async Task<ComprehensiveAnalysisDailySyncResult> ExecuteAsync(CancellationToken cancellationToken)
    {
        var sw = Stopwatch.StartNew();
        var now = timeProvider.GetUtcNow();
        var yesterday = DateOnly.FromDateTime(now.AddDays(-1).DateTime);
        var today = DateOnly.FromDateTime(now.DateTime);

        var logId = await repository.InsertSyncLogAsync("DailySync", cancellationToken);
        var totalPages = 0;
        var totalItems = 0;
        string? lastError = null;

        logger.LogInformation(
            "ComprehensiveAnalysis daily sync started for {From} to {To}.",
            yesterday,
            today);

        try
        {
            var firstPage = await client.GetPageAsync(1, yesterday, today, cancellationToken);
            if (firstPage is null)
            {
                await repository.UpdateSyncLogAsync(logId, 0, 0, "Failed", "API returned null for page 1.", cancellationToken);
            ...
            for (var page = 2; page <= lastPage; page++)
            {
                ...
                try
                {
                    var pageData = await client.GetPageAsync(page, yesterday, today, cancellationToken);
                    if (pageData is null)
                    {
                        logger.LogWarning("ComprehensiveAnalysis daily sync: page {Page} returned null, skipping.", page);
                        continue;
                    }
                ...
                {
                    logger.LogError(ex, "ComprehensiveAnalysis daily sync: error on page {Page}.", page);
            ...
            logger.LogInformation(
                "ComprehensiveAnalysis daily sync finished: {Pages} pages, {Items} items, {Duration}.",
                totalPages,
                totalItems,
                sw.Elapsed);
        }
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            ...
            logger.LogError(ex, "ComprehensiveAnalysis daily sync failed.");
            throw;
        }

        return new ComprehensiveAnalysisDailySyncResult(totalPages, totalItems, sw.Elapsed);
    }
