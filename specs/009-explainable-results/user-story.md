...
I want each structured financial AI answer to expose deterministic evidence and confidence,
...
- Each completed structured financial answer includes a top-level answer-level Confidence Score; per-result confidence is optional only when a documented policy calculates it separately.
...
- Scanner answers include explainable evidence, the top-level Confidence Score, and display backend-produced usage metadata from Billing required by the current chat UI; explainability does not calculate charges or balances.
- Symbol lookup answers include the same top-level Confidence Score even when no `ExplainableAnswer` object is present. The frontend must not infer `0%` from a missing scanner explainable answer when a structured `SymbolLookupTable` exists.
- The answer-level Confidence Score is calculated by a backend confidence scoring service from validated interpretation certainty, evidence completeness, source freshness, execution warnings, and, for symbol lookup, consistency between deterministic prose and structured table values.
...
- Structured evidence and the top-level Confidence Score are returned through `POST /api/ai/v1/query` for both our own UI and conversational external clients.
...
- In the Microsoft Agent Framework orchestration, confidence calculation is a required deterministic workflow function/executor after scanner, symbol lookup, or other structured financial result evidence exists and before the assistant Message is finalized. It is not an optional tool call selected by the LLM.
...
- The assistant Message persists answer evidence and the top-level Confidence Score needed for later Conversation retrieval.
