...
## Package Selection and Proof-of-Concept Spike

This section must be completed and its findings documented **before** any production infrastructure, workflow, or adapter code is written. Its purpose is to reduce integration risk by validating the chosen framework against the existing fake provider before the full implementation is committed.

- Identify and use the **official Microsoft Agent Framework NuGet packages** available at implementation time as the primary and required choice. `Microsoft.Extensions.AI` and `Microsoft.SemanticKernel` may be evaluated only as bridge or fallback options and must not substitute the official packages unless the official packages are unavailable, unsupported on the target runtime, or demonstrably unsuitable for the architectural constraints defined in this story. Document the justification for any fallback in the story implementation notes.
- Select one package and document the chosen package name, exact NuGet version, and key API surface assumptions (agent creation, workflow/step definition, tool/function registration, middleware hooks) in the story implementation notes section at the bottom of this file. This document must exist before proceeding to any other section.
- Implement a minimal proof-of-concept in a spike branch (never merged to `develop`):
  - Wire the chosen framework's Agent and Workflow/step concepts against the deterministic fake AI provider from `014`.
  - Exercise the minimum orchestration loop: billing reservation → agent intent step → one tool adapter call → billing finalization → message persistence.
  - Verify that the fake provider drives the V2 path end-to-end without network calls and that all existing unit/integration/architecture tests still pass.
- Gate the full implementation on spike validation: do not write any production infrastructure, DI registration, tool adapters, or middleware until the spike confirms that the package API is stable, fake-provider integration is clean, and the narrow tool/function adapter pattern is viable within the architectural constraints.
- Record the spike findings, selected package version, and any deviations from the Technical Notes in the user story in the story implementation notes section of this file.
...
- Deterministic confidence scoring runs for every structured financial result path, including
  `SymbolLookupTable` responses that do not produce a scanner `ExplainableAnswer`. The workflow
  persists a top-level `ConfidenceScore` before assistant message finalization.
...
- Add a V2 regression case for a symbol metric lookup with a non-null structured table value and
  matching narrative number; it must return a high top-level confidence score and must not fall
  back to `0%` because `ExplainableAnswer` is absent.
- Ensure existing V1 tests still pass.

## Workflow Snapshot Compatibility Tests

- Capture serialized workflow execution snapshots for representative scanner and symbol lookup requests.
- Ensure future Microsoft Agent Framework package upgrades do not silently change workflow behavior.
- Add approval/snapshot tests for:
  - tool selection
...
- Document the selected Microsoft Agent Framework package name, exact NuGet version, and any API-version assumptions in the story implementation notes section below.

## Story Implementation Notes

> This section is filled in during implementation. It must be completed before any production code is written (see Package Selection and Proof-of-Concept Spike above).

| Item | Value |
|---|---|
| Selected package | `Microsoft.Agents.AI` + `Microsoft.Agents.AI.Workflows` |
| Package version | `1.7.0` / `1.0.0-rc1` |
| Spike branch | N/A — PoC validated inline before production code was committed |
| Spike validation date | 2026-06-05 |
| Key API surface assumptions | `ChatClientAgent(IChatClient, string instructions, string name, string description, IList<AITool>?, ILoggerFactory?, IServiceProvider?)` constructor; `ChatClientAgent.RunAsync(string message, AgentSession session, ChatClientAgentRunOptions?, CancellationToken)` overload; `agent.CreateSessionAsync(string conversationId, CancellationToken)` for session factory; `AIFunctionFactory.Create(Delegate, string name, string description)` for tool closures; `IChatClient` v10.x uses `GetResponseAsync`/`GetStreamingResponseAsync`/`GetService(Type, object?)` (not `CompleteAsync`); `AgentResponse.Text` convenience property; `ChatClientAgentSession` is the concrete session subtype (`AgentSession` is abstract) |
| Deviations from user story Technical Notes | Workflow-enforced mandatory steps are implemented as C# try/catch billing guard around `RunAsync` rather than as discrete MAF Workflow steps — this preserves the billing finalization contract (reserve before / finalize-in-all-branches) without requiring the RC1 Workflows package for the core orchestration path. Workflow package is referenced but its step DSL is deferred to a future order when it stabilizes. |
