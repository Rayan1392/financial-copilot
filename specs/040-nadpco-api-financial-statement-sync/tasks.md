...
2. Define curated, reviewed item-ID mappings to canonical metrics and document units/scales.
...
4. Add `NadpcoApiFinancialStatementNormalizer` with distinct statement types, source-date
   mapping, provenance evidence, canonical-variant selection, and idempotent upserts.
...
7. Add tests for each statement type, item allowlists, period mapping, audited/represented/
   composing selection, idempotency, cross-provider coexistence, malformed payloads, and
...
- Added `NadpcoApiFinancialStatementNormalizer` with distinct `FinancialStatementType` rows,
  deterministic canonical variant selection, Gregorian period mapping, Jalali/variant evidence,
  idempotent upserts, and recalculation request publication.
