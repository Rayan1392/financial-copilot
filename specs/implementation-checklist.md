...
| 2026-05-26 | `013-billing-and-credits-domain` | Partially verified | Implemented Billing domain/contracts and service foundation plus EF Core/PostgreSQL persistence mappings and repositories for accounts, wallets, reservations, ledgers, subscriptions, and invoice profiles. Added bounded abandoned-reservation expiry recovery; optimistic revision protection for simultaneous wallet-projection writes; idempotent currency transaction accounting kept separate from usage credits; `GET /api/v1/usage/me` and tenant-resolved `GET /api/v1/billing/transactions`; and operation/outcome pricing coverage. Atomic multi-record execution finalization/distributed coordination, AI workflow/facade charging integration, administration, and payment/invoice execution remain. `dotnet test src/backend/FinancialCopilot.sln --configuration Release --no-restore` passed: Unit 33, Integration 27, Architecture 1. |
