...
- `045-symbol-metric-point-lookup` — answer direct questions about a specific symbol's metric value ("PE حفاری چقدر است؟") using a new `SymbolLookup` intent, `ISymbolLookupParser`, `ISymbolNameResolver`, and `ISymbolMetricLookupService`. Re-uses the scanner table contract so the frontend renders lookup results without a new component.
